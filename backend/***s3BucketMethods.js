...
const getBucketList = () => {
  ...
  return bucketData.then((data) => [data.Buckets])
    .catch((err) => {
      console.log(err);
    });
};
...
const createBucket = (bucketName, accessControl = 'private') => {
  ...
  return bucketData.then((data) => data)
    .catch((err) => {
      console.log(err);
    });
};
...
const getBucketRegion = (bucketName) => {
  ...
  return bucketRegion.then(({ LocationConstraint }) => LocationConstraint)
 ...
 * @return {object[]} return is a sorted array of objects
 */
function sortFileArray(fileArray) {
  return fileArray.sort(({ Key: a }, { Key: b }) => (a.match(/\//g) || []).length - (b.match(/\//g) || []).length);
}
