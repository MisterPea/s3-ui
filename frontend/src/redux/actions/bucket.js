...
export function getBucketList() {
  return (dispatch) => {
    ...
    axios({
      method: 'GET',
      url: `${HOSTNAME}/api/getBucketList`,
    }).then(({ data }) => {
      dispatch(hideLoader());
...
export function getBucketContentsList(region, bucket) {
  return (dispatch) => {
    ...
    axios({
      method: 'POST',
      url: `${HOSTNAME}/api/getBucketContents`,
...
export function getBucketsAndContentsList(region, bucket) {
  return (dispatch) => {
    ...
    axios({
      method: 'POST',
      url: `${HOSTNAME}/api/getBucketsAndContents`,
      data: JSON.stringify({ locale: region, bucket }),
      headers: {
        'content-type': 'application/json',
      },
