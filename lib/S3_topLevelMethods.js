...
function getAllBuckets() {
  ...
  return getBuckets(s3).then((data) => {
    return appendBucketRegion(s3, data.Buckets);
  });
}
...
function getBucketContents(locale, bucketName) {
  ...
  return s3.send(new ListObjectsV2Command({Bucket: bucketName}))
      ...
      .catch((err) => {
        logError('Getting bucket contents', err);
        return err;
      });
}
